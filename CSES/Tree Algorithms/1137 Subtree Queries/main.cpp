...
public:
    ...
    void solve()
    {
        ...
        for (int i = 1; i <= N; i++)
        {
            valsInDFSOrder[dfsTraversal[i]] = vals[i];
        }

        BIT = BinaryIndexedTree(valsInDFSOrder, valsInDFSOrder + N + 1);
        ...
        for (int i = 1; i <= Q; i++)
        {
            ...
            else if (t == 2)
            {
                ...
                printf("%lld\n", BIT.getRangeSum(dfsTraversal[node], dfsTraversal[node] + subtreeSize[node] - 1));
            }
        }
    }
}
