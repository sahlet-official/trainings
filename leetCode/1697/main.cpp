...
public:
    vector<bool> distanceLimitedPathsExist(int n, vector<vector<int>>& edgeList, vector<vector<int>>& queries) {
        ...
        sort(begin(edgeList), end(edgeList), [](vector<int>& l, vector<int>& r) {return l[2] < r[2];});
        sort(begin(queries), end(queries), [](vector<int>& l, vector<int>& r) {return l[2] < r[2];});
        ...
        for (int i = 0; i < queries.size(); i++)
        {
            ...
            {
                u.connect(edgeList[j][0], edgeList[j][1]);
                j++;
            }

            res[queries[i][3]] = u.connected(queries[i][0], queries[i][1]);
        }
...
void solve(std::istream& in, std::ostream& out)
{
    ...
    v1 = {{0,1,2},{1,2,4},{2,0,8},{1,0,16}};
    v2 = {{0,1,2},{0,2,5}};
    s.distanceLimitedPathsExist(3, v1, v2);
}
