...
class Solution {
    ...
    void countFMemory(int n, int k, int maxPts) {
        int maxNumber = min(maxX, k);

        fMemory.clear();
        fMemory.resize(maxNumber + 1);

        fMemory[0] = 1;

        // for (int i = 1; i <= maxNumber; i++) {
        //     for (int j = 1; j <= min(i, maxPts); j++) {
        //         fMemory[i] += fMemory[i - j];
        //     }

        //     fMemory[i] *= (1.0 / maxPts);
        // }

        for (int i = 1; i <= maxNumber; i++) {
            fMemory[i] +=
            fMemory[i - 1] / maxPts +
            ((i - 1) > 0 ? (fMemory[i - 1] - (i > maxPts ? (fMemory[i - (maxPts + 1)] / maxPts) : 0)) : 0);
        }
    }

public:
    double new21Game(int n, int k, int maxPts) {
        countFMemory(n, k, maxPts);
        ...
        if (k == 0)
        {
            //P += f(0) * min(maxPts, n) * (1.0 / maxPts);
            P += 1;
        }
        else
        for (int i = 1; i <= min(maxPts, k); i++) {
            P += f(k-i) * (min(maxPts, n - k + i) - (i - 1)) * (1.0 / maxPts);
        }
...
void solve(std::istream& in, std::ostream& out)
{
    ...
    out << (s.new21Game(10, 1, 10) - 1) << std::endl;
    out << (s.new21Game(6, 1, 10) - 0.6) << std::endl;
    out << (s.new21Game(21, 17, 10) - 0.73278) << std::endl;
    // out << (s.new21Game(1, 0, 1) - 1) << std::endl;
    // out << (s.new21Game(1, 0, 10) - 0.1) << std::endl;
    out << (s.new21Game(9367, 7346, 5344) - 0.62079) << std::endl;
}
