...
public:
    bool isTransformable(string s, string t) {
        std::vector<std::pair<std::vector<int>, int>> digits(10);
        
        for (size_t i = 0; i < digits.size(); i++)
        {
            digits[i].first.reserve(s.size());
        }
        
        for (size_t i = 0; i < s.size(); i++)
        {
            digits[s[i] - '0'].first.push_back(i);
        }
        ...
        for (size_t i = 0; i < t.size(); i++)
        {
            ...
            if (digits[digit].second >= digits[digit].first.size())
            {
                return false;
            }

            int digitPos = digits[digit].first[digits[digit].second];

            for (int smallerDigit = digit - 1; smallerDigit >= 0; smallerDigit--)
            {
                if (digits[smallerDigit].second >= digits[smallerDigit].first.size())
                {
                    continue;
                }

                int smallerDigitPos = digits[smallerDigit].first[digits[smallerDigit].second];
                if (smallerDigitPos < digitPos)
                {
                    return false;
                }
            }
...
void solve(std::istream& in, std::ostream& out)
{
    ...
    out << (s.isTransformable("84532", "34852") == true) << std::endl;
    out << (s.isTransformable("34521", "23415") == true) << std::endl;
    out << (s.isTransformable("12345", "12435") == false) << std::endl;
}
