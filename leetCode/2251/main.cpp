...
public:
    vector<int> fullBloomFlowers(vector<vector<int>>& flowers, vector<int>& people) {
        ...
        sorted.reserve(flowers.size() * 2 + people.size());

        for (size_t i = 0; i < flowers.size(); i++)
        {
            sorted.emplace_back(flowers[i][0], -1);
            sorted.emplace_back(flowers[i][1] + 1, -2);
        }

        for (size_t i = 0; i < people.size(); i++)
        {
            sorted.emplace_back(people[i], i);
        }
        ...
        for (size_t i = 0; i < sorted.size(); i++)
        {
            ...
            else if (sorted[i].second == -2)
            {
                flowersCount--;
            }
            else
            {
                res[sorted[i].second] = flowersCount;
            }
        }
