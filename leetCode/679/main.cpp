...
class Solution {
    inline bool isRes(double val)
    {
        return abs(val - 24) < 0.00001;
    }
    ...
    //     virtual double count() { return value; }
    // };
    ...
    bool recursion(int operandsNumber, std::function<bool (double /*left*/)> countNext) {
        // int unused = 0;
        ...
        for (int i = 1; i < operandsNumber; i++)
        {
            bool res = recursion(
                i, 
                [&](double left) -> bool
                {
                    return recursion(operandsNumber - i, [&](double right) -> bool
                    {
                        return countNext(left + right) ||
                        countNext(left - right) ||
                        countNext(left * right) ||
                        (abs(right) > 0.00001 ? countNext(left / right) : false);
                    });
                }
            );
...
public:
    bool judgePoint24(vector<int>& cards) {
        ...
        notUsed = std::vector<bool>(cards.size(), true);
        
        return recursion(
            4,
            [](double left) -> bool {
                if (abs(left - 24) < 0.00001)
                {
                    return true;
                }
                return false;
...
void solve(std::istream& in, std::ostream& out)
{
    ...
    v = {4,1,8,7};
    out << (s.judgePoint24(v) == true) << std::endl;
    v = {1,2,1,2};
    out << (s.judgePoint24(v) == false) << std::endl;
}
