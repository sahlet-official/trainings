...
public:
    int minSwapsCouples(vector<int>& row) {
        ...
        for (size_t i = 0; i + 2 < row.size(); i += 2)
        {
            int& left = row[i];
            int& right = row[i + 1];
            int leftPair = left % 2 ? left - 1 : left + 1;
            ...
            swap(right, row[places[leftPair]]);
            swap(places[leftPair], places[row[places[leftPair]]]);
...
void solve(std::istream& in, std::ostream& out)
{
    ...
    v = {10,7,4,2,3,0,9,11,1,5,6,8};
    out << (s.minSwapsCouples(v) == 4) << std::endl;
}
