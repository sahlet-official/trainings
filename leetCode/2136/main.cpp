...
public:
    int earliestFullBloom(vector<int>& plantTime, vector<int>& growTime) {
        int maxGrow = *max_element(growTime.begin(), growTime.end());
        std::vector<int> order(maxGrow + 1);
        for (size_t i = 0; i < growTime.size(); i++)
        {
            order[growTime[i]] += plantTime[i];
        }
        ...
        for (int i = order.size() - 1; i >= 0; i--)
        {
            ...
            growTimePrev = max(i, growTimePrev - plantTimeCur);

            plantTimePrev += plantTimeCur;
        }
        
        return plantTimePrev + growTimePrev;
    }
};
...
void solve(std::istream& in, std::ostream& out)
{
    ...
    v1 = {1,4,3};
    v2 = {2,3,1};
    out << (s.earliestFullBloom(v1, v2) == 9) << std::endl;
    v1 = {6,8,7,22,30,5,17,18};
    v2 = {30,15,1,21,5,13,5,11};
    out << (s.earliestFullBloom(v1, v2) == 114) << std::endl;
}
