...
public:
    int maxPerformance(int n, vector<int>& speed, vector<int>& efficiency, int k) {
        ...
        {
            sum += speed[order[i]];
            minEfficiency = min(minEfficiency, efficiency[order[i]]);
        }
        ...
        auto comparator = [](int lIndex, int rIndex) {
            if ()
        };

        priority_queue<int, std::vector<int>, decltype(comparator)> q(comparator);
        q
    }
};
