...
public:
    vector<vector<int>> matrixRankTransform(vector<vector<int>>& matrix) {
        std::vector<int /*rank*/> maxInRow(matrix.size(), 0);
        std::vector<int /*rank*/> maxInColumn(matrix[0].size(), 0);
        ...
        std::map<int, std::vector<CellInfo>> cellsM;

        for (size_t i = 0; i < matrix.size(); i++)
        {
            for (size_t j = 0; j < matrix[0].size(); j++)
            {
                cellsM[matrix[i][j]].emplace_back(i, j);
            }
        }

        vector<vector<int>>& res = matrix;

        for (auto& [value, cells] : cellsM)
        {
            std::unordered_map<int, std::vector<int>> unions;

            {
                // find unions and put to unions
                ...
                {
                    if (rows.count(cells[k].row))
                    {
                        u.connect(k, rows[cells[k].row]);
                    }
                    else
                    {
                        rows[cells[k].row] = k;
                    ...
                    if (columns.count(cells[k].column))
                    {
                        u.connect(k, columns[cells[k].column]);
                    }
                    else
                    {
                        columns[cells[k].column] = k;
                    }
                }

                for (size_t k = 0; k < cells.size(); k++)
                {
                    unions[u.find(k)].push_back(k);
                }
            }

            for (auto& [_, connected] : unions)
            {
                int maxRank = INT_MIN;

                for (int cellIndex: connected)
                {
                    maxRank = max(maxRank, maxInRow[cells[cellIndex].row]);
                    maxRank = max(maxRank, maxInColumn[cells[cellIndex].column]);
                }
                ...
                for (int cellIndex: connected)
                {
                    maxInRow[cells[cellIndex].row] = thisRank;
                    maxInColumn[cells[cellIndex].column] = thisRank;
                    
                    res[cells[cellIndex].row][cells[cellIndex].column] = thisRank;
                }
            }
        }
