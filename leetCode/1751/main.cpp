...
public:
    int maxValue(vector<vector<int>>& events, int k) {
        ...
        std::vector<std::vector<int>> dp(events.size(), std::vector<int>(k + 1, -1));

        int maxV = INT_MIN;
        for (int i = int(events.size()) - 1; i >= 0; i--)
        {
            maxV = max(maxV, events[i][2]);
            dp[i][1] = maxV;
        }

        for (int j = 1; j <= k; j++)
        {
            dp[int(events.size()) - 1][j] = events.back()[2];
        }

        vector<int> forSearch(3);
        for (int i = int(events.size()) - 2; i >= 0; i--)
        {
            forSearch[0] = events[i][1];
            int iNext = std::upper_bound(events.begin() + i, events.end(), forSearch, comp) - events.begin();
            for (int j = 2; j <= k; j++)
            {
                int nextVal = iNext < events.size() ? dp[iNext][j - 1] : 0;
                dp[i][j] = max(nextVal + events[i][2], dp[i + 1][j]);
            }
        }
