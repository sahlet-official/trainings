...
const int primeNumbersSieveSize = 100001;
...
public:
    bool gcdSort(vector<int>& nums) {
        ...
        Union u(primeNumbersSieveSize);
        for (size_t i = 0; i < nums.size(); i++)
        {
            int y = nums[i];
            for (int primeNumber : primeNumbers)
            {
                if (primeNumber * primeNumber > y)
                {
                    break;
                }
        ...
        for (size_t i = 0; i < nums.size(); i++)
        {
            if (!u.connected(sorted[i], nums[i]))
            {
                return false;
            }
        }
