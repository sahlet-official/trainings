...
public:
    ...
    int howManySameLength(vector<string>& digits, int n)
    {
        if (n <= 0)
        {
            return 0;
        ...
        for (int i = length; i > 0; i--)
        {
            ...
            char firstDigit = n / divider + '0';

            for (size_t i = 0; i < digits.size(); i++)
            {
                if (digits[i][0] >= firstDigit)
                {
                    if (digits[i][0] == firstDigit)
                    {
                        hasSameDigit = true;
    ...
    int atMostNGivenDigitSet(vector<string>& digits, int n) {
        int res = howManySameLength(digits, n);
...
void solve(std::istream& in, std::ostream& out)
{
    ...
    v = {"1", "2"};
    out << (s.howManySameLength(v, 2) == 2) << std::endl;
    v = {"1", "2"};
    out << (s.howManySameLength(v, 10) == 0) << std::endl;
    out << (s.howManySameLength(v, 11) == 1) << std::endl;
    out << (s.howManySameLength(v, 22) == 4) << std::endl;
    out << (s.howManySameLength(v, 331) == 8) << std::endl;
    out << (s.howManySameLength(v, 133) == 4) << std::endl;
}
