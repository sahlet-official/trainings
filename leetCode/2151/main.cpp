...
class Solution {
    ...
    void bt(vector<vector<int>>& statements, std::vector<int>& memory, int curRes, int undefinedPeople, int personIndex)
    {
        if (curRes + undefinedPeople <= maxRes)
        {
            return;
        }

        if(personIndex >= memory.size())
        {
            maxRes = max(maxRes, curRes);
            return;
        }

        if (memory[personIndex] == -1 || memory[personIndex] == 1)
        {
            int memoryCopy[memory.size()];
            for (size_t i = 0; i < memory.size(); i++)
            {
                memoryCopy[i] = memory[i];
            }
            
            int undefinedPeopleCopy = undefinedPeople;
            int curResCopy = curRes;
            ...
            auto discard = [&]()
            {
                ...
                undefinedPeople = undefinedPeopleCopy;
                curRes = curResCopy;
            };
            ...
            {
                std::function<void()> f;
                Discarder(std::function<void()> f) : f(f)
                {
            ...
            bool canBeGood = true;
            if (memory[personIndex] == -1)
            {
                memory[personIndex] = 1;
                undefinedPeople--;
                curRes++;
            }
            ...
            {
                if (statements[personIndex][i] == 0 || statements[personIndex][i] == 1)
                {
                    if (memory[i] == -1)
                    {
                        memory[i] = statements[personIndex][i];
                        undefinedPeople--;
                        if (memory[i] == 1)
                        {
                            curRes++;
                    ...
                    else if (memory[i] != statements[personIndex][i])
                    {
                        canBeGood = false;
                        break;
                    }
            ...
            if (canBeGood)
            {
                bt(statements, memory, curRes, undefinedPeople, personIndex + 1);
            }
...
public:
    int maximumGood(vector<vector<int>>& statements) {
        ...
        std::vector<int> memory(statements.size(), -1);
        bt(statements, memory, 0, statements.size(), 0);
...
void solve(std::istream& in, std::ostream& out)
{
    ...
    v = {{2,1,2},{1,2,2},{2,0,2}};
    out << (s.maximumGood(v) == 2) << std::endl;
    v = {{2,0},{0,2}};
    out << (s.maximumGood(v) == 1) << std::endl;
    v = {{2,0,2,2,0},{2,2,2,1,2},{2,2,2,1,2},{1,2,0,2,2},{1,0,2,1,2}};
    out << (s.maximumGood(v) == 2) << std::endl;
    v = {
        {2,2,2,0,0,0,2,2,0,0},
        {2,2,0,2,2,2,2,2,0,0},
        {0,2,2,0,1,2,1,2,1,0}, //0
        {2,1,0,2,1,2,1,1,1,1}, //0
        {2,2,1,2,2,1,2,1,0,0}, //0
        {2,1,2,1,1,2,1,1,2,2}, //0
        {2,2,2,2,2,2,2,1,1,0}, //0
        {1,0,1,2,0,1,0,2,2,0}, //0
        {1,0,0,0,2,2,0,0,2,0}, //0
        {1,0,1,2,0,0,2,2,2,2}}; //0
    out << (s.maximumGood(v) == 2) << std::endl;
        v = {
        {2,1,0,0,2}, //0
        {2,2,1,0,2}, //0
