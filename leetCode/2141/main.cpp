...
public:
    long long maxRunTime(int n, vector<int>& batteries) {
        ...
        for (int i = batteries.size() - n, j = 0; i < batteries.size(); i++, j++)
        {
            live[j] = batteries[i];
        }
        ...
        int liveIndex = 0;
        for (; liveIndex + 1 < n; liveIndex++)
        {
            long long dif = (live[liveIndex + 1] - live[liveIndex]) * (liveIndex + 1);
        ...
        return live[liveIndex] + (extra / (liveIndex + 1));
    }
};
...
void solve(std::istream& in, std::ostream& out)
{
    ...
    v = {11,89,16,32,70,67,35,35,31,24,41,29,6,53,78,83};
    out << (s.maxRunTime(12, v) == 43) << std::endl;
    v = {3,3,3};
    out << (s.maxRunTime(2, v) == 4) << std::endl;
}
