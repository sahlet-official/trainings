...
class Solution {
    ...
    int changeBit(int i)
    {
        ...
        if (!(n & (1 << (i - 1))))
            ...
            {
                int nextNonZeroBit = i - 2;
                while (nextNonZeroBit >= 0 && !(n & (1 << nextNonZeroBit)))
                {
                    nextNonZeroBit--;
                }
...
void solve(std::istream& in, std::ostream& out)
{
    ...
    v = {2, 3};
    out << (s.minimumOneBitOperations(3) == 2) << std::endl;
    out << (s.minimumOneBitOperations(6) == 4) << std::endl;
}
