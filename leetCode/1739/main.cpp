...
public:
    int minimumBoxes(int n) {    
        ...
        if (memory.empty())
        {
            ...
            {
                height++;
                memory[height] = memory[height - 1] + (memory[height - 1] - memory[height - 2]) + height;
            }
            while (memory[height] <= 1e9);
        }

        int heightL = 0;
        int heightR = memory.size() - 1;

        while (heightR != (heightL + 1))
        {
            int index = heightL + (heightR - heightL) / 2;
            if (n >= memory[index])
            {
                heightL = index;
            }
        ...
        int res = heightL * (heightL + 1) / 2;
        int rest = n - memory[heightL];
...
void solve(std::istream& in, std::ostream& out)
{
    ...
    v = {2, 3};
    out << (s.minimumBoxes(3) == 3) << std::endl;
    out << (s.minimumBoxes(4) == 3) << std::endl;
    out << (s.minimumBoxes(10) == 6) << std::endl;
}
