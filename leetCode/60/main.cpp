...
public:
    string getPermutation(int n, int k) {
        ...
        for (int i = 0; i < n; i++)
        {
            long long space = rangeR - rangeL;
            long long step = space / (n - i);
            
            int steps = (k - rangeL - 1) / step;
...
void solve(std::istream& in, std::ostream& out)
{
    ...
    v = {2, 3};
    out << (s.getPermutation(3, 3) == "213") << std::endl;
    out << (s.getPermutation(4, 9) == "2314") << std::endl;
    out << (s.getPermutation(3, 1) == "123") << std::endl;
}
