...
public:
    int minimumDeviation(vector<int>& nums) {
        ...
        for (size_t i = 0; i < nums.size(); i++)
        {
            while (nums[i] % 2 == 0 && nums[i] / 2 >= minN)
            {
                nums[i] >>= 1;
            }
        }
        ...
        int difference = nums.back() - nums.front();
        
        int i = nums.size() - 1;
        while (nums[i] % 2 == 0)
        {
            minN = nums[i] /= 2;
            i = i > 0 ? i - 1 : nums.size() - 1;
            difference = min(difference, nums[i] - minN);
        }
...
void solve(std::istream& in, std::ostream& out)
{
    ...
    v = {3, 5};
    out << (s.minimumDeviation(v) == 1) << std::endl;
}
