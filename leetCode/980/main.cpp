...
class Solution {
    ...
    void count(vector<vector<int>>& grid, int x, int y)
    {
        ...
        for (auto pair : possibleMoves)
        {
            ...
            {
                grid[pair.first][pair.second] = 3;
                moves++;
                count(grid, pair.first, pair.second);
                grid[pair.first][pair.second] = 0;
                moves--;
            }
            else if (grid[pair.first][pair.second] == 2)
            {
                if (moves == emptySquares)
                {
                    res++;
                }
            }
