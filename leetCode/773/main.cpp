...
class Solution {
    ...
    std::unordered_map<string, int /*distance*/> memory;

    void BFS(string sourceBoard)
    {
        std::queue<string> queue;

        queue.push(sourceBoard);

        while(!queue.empty())
        {
            auto& front = queue.front();
            int curentDistance = memory[front];
            
            int zeroPos = front.find('0');

            for (auto& otherPos : moves[zeroPos])
            {
                swap(front[zeroPos], front[otherPos]);

                if (memory.count(front) == 0)
                {
                    memory[front] = curentDistance + 1;
                    queue.push(front);
                }
...
public:
    int slidingPuzzle(vector<vector<int>>& board) {
        ...
        return memory.count(str) ? memory[str] : -1;
    }
};
...
void solve(std::istream& in, std::ostream& out)
{
    ...
    v = {{1,2,3},{4,0,5}};
    out << (s.slidingPuzzle(v) == 1) << std::endl;
    v = {{1,2,3},{5,4,0}};
    out << (s.slidingPuzzle(v) == -1) << std::endl;
    v = {{4,1,2},{5,0,3}};
    out << (s.slidingPuzzle(v) == 5) << std::endl;
}
