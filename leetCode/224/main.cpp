...
class Solution {
    ...
    Res calculateSpace(int start) {
        ...
        while (start < s.size())
        {
            ...
            auto subres = calculate(start);
            start = subres.endIndex;
            res += subres.value;
        }
    ...
    Res calculate(int start) {
        ...
        else if (s[start] == '+')
        {
            ...
            return {subRes.endIndex, subRes.value};
        }
        else if (s[start] == '-')
        {
            start++;
        ...
        else if (s[start] == '(')
        {
            ...
            auto subRes = calculateSpace(start);

            if (subRes.endIndex < s.size() && s[subRes.endIndex] == ')')
            {
                start = subRes.endIndex + 1;
                return {start, subRes.value};
            }
            else
            {
                return {start, 0};
...
void solve(std::istream& in, std::ostream& out)
{
    ...
    v = {2, 3};
    out << (s.calculate("1 + 1") == 2) << std::endl;
    out << (s.calculate("2-1 + 2") == 3) << std::endl;
    out << (s.calculate("(1+(4+5+2)-3)+(6+8)") == 23) << std::endl;
}
